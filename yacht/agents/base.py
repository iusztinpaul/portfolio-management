...
class BaseAgent:
    ...
    def load_network(self) -> int:
        ...
        if len(checkpoints) > 0:
            ...
            raise RuntimeError(f'No checkpoint to load from {self.model_path}. Maybe set "resume" training to "false"')
...
class BackTestBaseAgent(BaseAgent):
    def __init__(
        ...
        super().__init__(environment, config, storage_path)

        for loader in environment.loaders.values():
            assert loader.get_batch_size() == 1, 'Back testing should be done only with "batch_size=1"'

        self.portfolio_value = 1
        self.portfolio_values = [
            (config.input_config.start_datetime - config.input_config.data_frequency.timedelta, self.portfolio_value)
        ]
